...
# Secure Login
	...
	if ($config{'autoRestart'} && time - $KoreStartTime > $config{'autoRestart'} && ($remote_socket && $remote_socket->connected()) && $conState == 5 
...
#add waitingTime
	if ($config{'waitingTimeStart'} ne "" && $config{'waitingTimeStop'} ne "" && ($remote_socket && $remote_socket->connected())) {
		...
		if ($dtime eq $config{'waitingTimeStart'}) {
			...
			sysLog("a", "waiting Time : $config{'waitingTimeStart'} to $config{'waitingTimeStop'}\n"); 
			$timeout_ex{'master'}{'time'} = time;
...
# Quit when trying reconnect more than dcOnTryReConnect
	if ($config{'dcOnTryReConnect'} && $conState_tries >= $config{'dcOnTryReConnect'}) {
		print "Trying Reconnect more than $config{'dcOnTryReConnect'} , Quiting ...\n";
		sysLog("a","Trying Reconnect more than $config{'dcOnTryReConnect'} , Quiting ...\n");
		quit();
	}
}
...
#Check if in special state
	...
	if (!$sys{'Xmode'} && $conState == 2 && $waitingForInput) {
		$config{'server'} = $input;
		$waitingForInput = 0;
		writeDataFileIntact("$def_config/config.txt", \%config);
	} elsif (!$sys{'Xmode'} && $conState == 3 && $waitingForInput) {
		$config{'char'} = $input;
		$waitingForInput = 0;
		writeDataFileIntact("$def_config/config.txt", \%config);
		sendCharLogin(\$remote_socket, $config{'char'});
...
#force command
	...
	#Cart command - chobit andy 20030101
		...
		if ($arg1 eq "") {
			...
			sendCartAddFromInv(\$remote_socket, $chars[$config{'char'}]{'inventory'}[$arg2]{'index'}, $arg3);
			...
			sendCartGetToInv(\$remote_socket, $arg2, $arg3);
...
#Sraet Chat Viewer
		...
		sysLog_clear("logs\/$config{'username'}");
		print qq~All log cleared.\n~; 
		...
		if (!%currentDeal) {
			...
			print sprintf("Zenny: %-14d            Zenny: %-14d\n",$you_string,$other_string);
			print "----------------------------------\n";
		}
		...
		if($arg1 eq ""){ 
			...
			print "BaseExp      : $totalBaseExp $percentB\n"; 
			print "JobExp       : $totalJobExp $percentJ\n"; 
			print "BaseExp/Hour : $bExpPerHour\n"; 
			print "JobExp/Hour  : $jExpPerHour\n";
			print "Base Levelup Time Estimation : $EstB_hour:$EstB_min:$EstB_sec\n";
			print "Job Levelup Time Estimation : $EstJ_hour:$EstJ_min:$EstJ_sec\n";
			print "----------------------------------\n"; 
			print "#   Name                    Amount\n"; 
			for ($i=0; $i<@monstersKilledID; $i++) { 
				next if ($monstersKilledID[$i] eq ""); 
				print sprintf("%-3d %-23s %6d\n", $i, $monstersKilled{$monstersKilledID[$i]}{'name'}, $monstersKilled{$monstersKilledID[$i]}{'count'}); 
			...
			print    "Error in function 'exp' (Exp Report)\n"
			,"Usage: exp [reset]\n";
		}
...
# Def-Con LogView
		...
		my ($arg1) = $input =~ /^[\s\S]*? (\w+)/;
		if ($arg1 eq "i" && -e "logs\/$config{'username'}_Items.txt") {
			print "\n----------Items Log---------\n";
			my (@printItem, %sItem , $line,$tItem);
			open (TMPMONSTI, "logs\/$config{'username'}_Items.txt"); 
			...
			foreach $tItem (sort keys %sItem) { 
				print sprintf("%-18s x %6d\n",$tItem,$sItem{$tItem});
			}
			close (TMPMONSTI); 
			print "-------------------------\n";
		} elsif ( $arg1 eq "m" && -e "logs\/$config{'username'}_Monsters.txt") {
			my (@printMonster,%sMonster,$totalmon,$line,$tMonster);
			print "\n------Monster Log------\n"; 
			$totalmon=0;
			open (TMPMONSTI, "logs\/$config{'username'}_Monsters.txt"); 
			while ($line = <TMPMONSTI>) { 
				@printMonster = split(/\n/, $line); 
			...
			sysLog_clear("logs\/$config{'username'}");
			print qq~System log cleared.\n~;
		} else {
			print "Error log file not found.\n";
			print "Syntax Error in function 'log' (Log Viewer)\n"
			,"Usage : log < i | m | c >\n";
		}
...
# Force StorageAuto & SellAuto
		...
		if (!$skill_num) {
			print "Syntax Error in function 'sl' (Use Skill on Location)\n",
		...
		if ($arg1 ne "str" &&  $arg1 ne "agi" && $arg1 ne "vit" && $arg1 ne "int" 
			...
			if ($chars[$config{'char'}]{"points_$arg1"} > $chars[$config{'char'}]{'points_free'}) {
				...
				print "Reach to Maximum Stat Limit (99)\n";
			}
		}
		...
		if ($arg1 eq "") {
			...
			sendStorageAddFromInv(\$remote_socket, $chars[$config{'char'}]{'inventory'}[$arg2]{'index'}, $arg3);
			...
			sendStorageGetToInv(\$remote_socket, $arg2, $arg3);
...
sub attack {
	...
	print addTag($sys{'Vx_interface'},"attackmons"),"Attacking: $monsters{$ID}{'name'} ($monsters{$ID}{'binID'})\n";
	injectMessage("Attacking: $monsters{$ID}{'name'} ($monsters{$ID}{'binID'})") if ($config{'verbose'} && $sys{'Xmode'});
#Mod Start
AUTOEQUIP: {
	my $i = 0;
	...
	while ($config{"autoSwitch_$i"} ne "") { 
		if (existsInList($config{"autoSwitch_$i"}, $monsters{$ID}{'name'})) {
			...
			injectMessage("Encounter Monster : $monsters{$ID}{'name'}") if ($config{'verbose'} && $sys{'Xmode'});
			...
			if ($Leq ne "" && !$chars[$config{'char'}]{'inventory'}[$Leq]{'equipped'}) { 
				...
				print "Auto Equiping [L] :".$config{"autoSwitch_$i"."_LeftHand"}." ($Leq)\n";
				injectMessage("Auto Equiping [L] :".$config{"autoSwitch_$i"."_LeftHand"}." ($Leq)") if ($config{'verbose'} && $sys{'Xmode'});
			...
			if ($Req ne "" && !$chars[$config{'char'}]{'inventory'}[$Req]{'equipped'} || $config{"autoSwitch_$i"."_RightHand"} eq "[NONE]") {
				...
				if ($config{"autoSwitch_$i"."_RightHand"} ne "[NONE]") {
					print "Auto Equiping [R] :".$config{"autoSwitch_$i"."_RightHand"}."($Req)\n"; 
					injectMessage("Auto Equiping [R] :".$config{"autoSwitch_$i"."_RightHand"}."($Req)") if ($config{'verbose'} && $sys{'Xmode'});
			...
			if ($arrow ne "" && !$chars[$config{'char'}]{'inventory'}[$arrow]{'equipped'}) { 
				print "Auto Equiping [A] :".$config{"autoSwitch_$i"."_Arrow"}."\n";
				injectMessage("Auto Equiping [A] :".$config{"autoSwitch_$i"."_Arrow"}) if ($config{'verbose'} && $sys{'Xmode'});
			...
			if ($config{"autoSwitch_$i"."_Distance"} && $config{"autoSwitch_$i"."_Distance"} != $config{'attackDistance'}) { 
				...
				injectMessage("Change Attack Distance to : ".$config{'attackDistance'}) if ($config{'verbose'} && $sys{'Xmode'});
			}
			if ($config{"autoSwitch_$i"."_useWeapon"} ne "") { 
				...
				injectMessage("Change Attack useWeapon to : ".$config{'attackUseWeapon'}) if ($config{'verbose'} && $sys{'Xmode'});
			}
			last AUTOEQUIP; 
		}
		$i++;
	...
	if ($config{'autoSwitch_default_LeftHand'}) { 
		...
		if($Leq ne "" && !$chars[$config{'char'}]{'inventory'}[$Leq]{'equipped'}) {
			...
			print "Auto equiping default [L] :".$config{'autoSwitch_default_LeftHand'}."\n";
			injectMessage("Auto equiping default [L] :".$config{'autoSwitch_default_LeftHand'}."($Leq)") if ($config{'verbose'} && $sys{'Xmode'});
			sendEquip(\$remote_socket, $chars[$config{'char'}]{'inventory'}[$Leq]{'index'},$chars[$config{'char'}]{'inventory'}[$Leq]{'type_equip'});
		}
	}
	if ($config{'autoSwitch_default_RightHand'}) { 
		...
		if($Req ne "" && !$chars[$config{'char'}]{'inventory'}[$Req]{'equipped'}) {
			print "Auto equiping default [R] :".$config{'autoSwitch_default_RightHand'}."\n"; 
			injectMessage("Auto equiping default [R] :".$config{'autoSwitch_default_RightHand'}."($Req)") if ($config{'verbose'} && $sys{'Xmode'});
			sendEquip(\$remote_socket, $chars[$config{'char'}]{'inventory'}[$Req]{'index'},$chars[$config{'char'}]{'inventory'}[$Req]{'type_equip'});
		}
	}
	if ($config{'autoSwitch_default_Arrow'}) { 
		...
		if($arrow ne "" && !$chars[$config{'char'}]{'inventory'}[$arrow]{'equipped'}) {
			print "Auto equiping default [A] :".$config{'autoSwitch_default_Arrow'}."\n"; 
			injectMessage("Auto equiping default [A] :".$config{'autoSwitch_default_Arrow'}."($arrow)") if ($config{'verbose'} && $sys{'Xmode'});
			sendEquip(\$remote_socket, $chars[$config{'char'}]{'inventory'}[$arrow]{'index'},0);
		}
	}
	if ($ai_v{'attackDistance'} && $config{'attackDistance'} != $ai_v{'attackDistance'}) { 
		...
		injectMessage("Change Attack Distance to default : ".$config{'attackDistance'}) if ($config{'verbose'} && $sys{'Xmode'});
	}
	if ($ai_v{'attackUseWeapon'} ne "" && $config{'attackUseWeapon'} != $ai_v{'attackUseWeapon'}) { 
		...
		injectMessage("Change Attack useWeapon to default : ".$config{'attackUseWeapon'}) if ($config{'verbose'} && $sys{'Xmode'});
	}
...
sub useTeleport { 
	...
	undef $teleQueue; 
	if (!$chars[$config{'char'}]{'ban_period'} && ($chars[$config{'char'}]{'skills'}{'AL_TELEPORT'}{'lv'} || $invIndex ne "")) {
		...
		configModify("saveMap_warpToBuyOrSell",0) if ($level == 2 && $config{'saveMap_warpToBuyOrSell'});
	}elsif ($chars[$config{'char'}]{'ban_period'}){
		print "[Warn] You are in Ban period ($chars[$config{'char'}]{'ban_period'} min), Can't use Skill / item\n";
		sysLog("warn","You are in Ban period ($chars[$config{'char'}]{'ban_period'} min), Can't use Skill / item\n");
	}
} 
#mod Stop
...
#chatLog -> Def-Con sysLog 
#original mod use many clock at once when add only one message ( collect sum and reprint all message )
#then modify to less cpu usage by only add one line to file when incomming message. Large increase file size is user repond.
sub sysLog {
	my $type = shift;
	my $message = shift;
# Seperate ChatLog
	if ( $type eq "s") {
		alertsound($config{'alertSound_onGMnotice'},$config{'alertSound_volume'}) if ($config{'alertSound'} && $^O eq 'MSWin32' && $config{'alertSound_onGMnotice'});
		open CHAT, ">> logs\/$config{'username'}_GMMessage.txt";
		print CHAT "[".getFormattedDate(int(time))."][".uc($type)."] $message";
	} elsif ( $type eq "i") {
		alertsound($config{'alertSound_onItem'},$config{'alertSound_volume'}) if ($message =~ /\*\*/ && $config{'alertSound'} && $^O eq 'MSWin32' && $config{'alertSound_onItem'});
		...
		alertsound($config{'alertSound_onShop'},$config{'alertSound_volume'}) if ($config{'alertSound'} && $^O eq 'MSWin32' && $config{'alertSound_onShop'});
		open CHAT, ">> logs\/$config{'username'}_Shop.txt";
		print CHAT "[".getFormattedDate(int(time))."] $message";
	} else {
		alertsound($config{'alertSound_onDanger'},$config{'alertSound_volume'}) if ($config{'alertSound'} && $^O eq 'MSWin32' && $config{'alertSound_onDanger'} && $type eq "D");
		alertsound($config{'alertSound_onPM'},$config{'alertSound_volume'}) if ($config{'alertSound'} && $^O eq 'MSWin32' && $config{'alertSound_onDanger'} && $type eq "pm");
...
sub dumpData {
	...
	if ($mode) {
		open DUMP, ">> logs\/$config{'username'}_sendDUMP.txt";
	}else{
		open DUMP, ">> logs\/$config{'username'}_recvDUMP.txt";
	}
	print DUMP $dump;
	close DUMP;
	print "$dump\n" if $config{'debug'} >= 2;
			...
			foreach (keys %{$mapPortals{$map}}) {
				...
				if ($portals_los{$portal}{$_} eq "" && $portals_los{$_}{$portal} eq "") {
					if ($field{'name'} ne $map) {
						...
						getField("$def_field/$map.fld", \%field);
					}
...
sub RespawnUnstuck {
	...
	sysLog("stuck", "Cannot calculate route, respawning to saveMap ...\n"); 
	aiRemove("move");
	aiRemove("route");
	aiRemove("route_getRoute");
	aiRemove("route_getMapRoute");
...
sub checkNPC{
	...
	foreach (@npcsID) { 
		...
		if($npcs_lut{$config{$position}}{'map'} eq $field{'name'} 
			...
			print "**Auto-Update $position : $position\n";
			sysLog("C", "**Auto-Update NPC ID : $position\n"); 
			last; 
		} 
	}
}
...
sub openShop {
	...
	if($chars[$config{'char'}]{'skills'}{'MC_VENDING'}{'lv'} && $shop{'shop_title'} ne ""){
		...
		print "Can not  open shop ( no skill to use or Empty Shop name )\n";
		shopconfigModify("shop_autoStart",0) if ($shop{'shop_autoStart'});
	}
}
...
sub changeDirection {
	...
	print "Item:($$r_hash2{'x'},$$r_hash2{'y'}) Char:($$r_hash1{'x'},$$r_hash1{'y'}) Look to:$direction\n" if ($config{'debug'});
...
sub attackStop {
	my $r_socket = shift;
	my $ID = shift;
	print "[Rep] Stop Attack : $monsters{$ID}{'name'} ($monsters{$ID}{'binID'})\n";
	shift @ai_seq;
	shift @ai_seq_args;
