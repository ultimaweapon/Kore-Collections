...
sub AI {
	...
	if (!$sys{'Xmode'} && timeOut(\%{$timeout{'ai_sync'}})) {
	...
	if (!$accountID) {
		$AI = 0;
		injectAdminMessage("Kore does not have enough account information, so AI has been disabled. Relog to enable AI.") if ($config{'verbose'} && $sys{'Xmode'});
		return;
	}
	...
	if (%cmd) {
		...
		if ($overallAuth{$cmd{'user'}} >= 1 
			...
			if ($cmd{'msg'} =~ /\bsit\b/i) {
				...
				sendMessage(\$remote_socket, $cmd{'type'}, getResponse("statS"), $cmd{'user'}) if $config{'verbose'};
				$timeout{'ai_thanks_set'}{'time'} = time;
...
#mod Stop
	...
	if ($sys{'Xmode'} && !$sentWelcomeMessage && timeOut(\%{$timeout{'welcomeText'}})) {
		injectAdminMessage($welcomeText) if ($config{'verbose'} && $sys{'Xmode'});
	...
	if ($ai_seq[0] eq "clientSuspend" && timeOut(\%{$ai_seq_args[0]})) {
		shift @ai_seq;
		shift @ai_seq_args;
	} elsif ($ai_seq[0] eq "clientSuspend" && $sys{'Xmode'}) {
...
# getAuto Part I
	...
	if ($ai_seq[0] eq "storageAuto" && $ai_seq_args[0]{'done'}) {
		...
		if ($ai_v{'temp'}{'arrow'}){
			sendEquip(\$remote_socket, $chars[$config{'char'}]{'inventory'}[$ai_v{'temp'}{'arrow'}]{'index'},0); 
...
#npc Step
			if (!$ai_seq_args[0]{'sentTalk'}) {
				...
				if ($ai_v{'temp'}{'arrow'} ne "") {
					sendUnequip(\$remote_socket,$chars[$config{'char'}]{'inventory'}[$ai_v{'temp'}{'arrow'}]{'index'});
				}
...
#getAuto Part II
			if (!$ai_seq_args[0]{'getStart'}) {
				for (my $i = 0; $i < @{$chars[$config{'char'}]{'inventory'}};$i++) {
					...
					if (($items_control{'all'}{'storage'} && $chars[$config{'char'}]{'inventory'}[$i]{'amount'} > $items_control{'all'}{'keep'} && !%{$items_control{lc($chars[$config{'char'}]{'inventory'}[$i]{'name'})}})
						...
						sendStorageAddFromInv(\$remote_socket, $chars[$config{'char'}]{'inventory'}[$i]{'index'}, $chars[$config{'char'}]{'inventory'}[$i]{'amount'} - $items_control{lc($chars[$config{'char'}]{'inventory'}[$i]{'name'})}{'keep'});
						$timeout{'ai_storageAuto'}{'time'} = time;
						last AUTOSTORAGE;
					}
				}
			...
			sendStorageGetToInv(\$remote_socket, $ai_seq_args[0]{'storageInvIndex'}, $getAmount); 
			$timeout{'ai_storageAuto'}{'time'} = time;
#mod Stop
		}
	}
	...
	if ($ai_seq[0] eq "buyAuto" && $ai_seq_args[0]{'done'}) {
		...
		if ($ai_v{'temp'}{'do_route'}) {
			...
			if ($config{'saveMap'} ne "" && $field{'name'} ne $config{'saveMap'} && $config{'saveMap_warpToBuyOrSell'} && !$ai_seq_args[0]{'warpedToSave'} && !$cities_lut{$field{'name'}.'.rsw'}) {
				...
				injectMessage(qq~Calculating auto-buy route to: $maps_lut{$npcs_lut{$config{"buyAuto_$ai_seq_args[0]{'index'}"."_npc"}}{'map'}.'.rsw'}($npcs_lut{$config{"buyAuto_$ai_seq_args[0]{'index'}"."_npc"}}{'map'}): $npcs_lut{$config{"buyAuto_$ai_seq_args[0]{'index'}"."_npc"}}{'pos'}{'x'}, $npcs_lut{$config{"buyAuto_$ai_seq_args[0]{'index'}"."_npc"}}{'pos'}{'y'}~) if ($config{'verbose'} && $sys{'Xmode'});
	...
	if ($ai_seq[0] eq "" && $field{'name'} && $config{"allowableMap"} ne "" && !(existsInList($config{"allowableMap"},$field{'name'}))) {
		print "You are out of allowable map : $field{'name'} \n";
		sysLog("D","You are out of allowable map : $field{'name'} \n");
		ai_setSuspend(5);
		if ($config{'reactallowableMap'} == 1) {
			print "Respawn to save map\n";
			sysLog("D","Respawn to save map\n");
			useTeleport(2);
		} elsif ($config{'reactallowableMap'} == 2) {
			print "Disconnect for the first place\n";
			sysLog("D","Disconnect for the first place\n");
			quit();
		}
	}
	...
	# Calculate lockMap Slot
		...
		#wrap around lockMap
			...
			print "lockMap timeOut switch map to :",$config{"lockMap_$ai_v{'lockMapIndex'}"},"\n";
	...
	# normal LockMap
		if ($field{'name'} ne $config{"lockMap_$ai_v{'lockMapIndex'}"} || ($config{"lockMap_$ai_v{'lockMapIndex'}"."_x"} ne ""
			...
			if ($maps_lut{$config{"lockMap_$ai_v{'lockMapIndex'}"}.'.rsw'} eq "") {
				print "Invalid map specified for lockMap - map".$config{"lockMap_$ai_v{'lockMapIndex'}"}."doesn't exist\n";
				injectMessage("Invalid map specified for lockMap - map ".$config{"lockMap_$ai_v{'lockMapIndex'}"}."doesn't exist") if ($config{'verbose'} && $sys{'Xmode'});
				...
				if ($config{"lockMap_$ai_v{'lockMapIndex'}"."_x"} ne "" || $config{"lockMap_$ai_v{'lockMapIndex'}"."_y"} ne "") {
					...
					print addTag($sys{'Vx_interface'},"route"),qq~Calculating lockMap route to: $maps_lut{$config{"lockMap_$ai_v{'lockMapIndex'}"}.'.rsw'}($config{"lockMap_$ai_v{'lockMapIndex'}"}): $ai_v{'temp'}{'randX'}, $ai_v{'temp'}{'randY'}\n~;
					injectMessage("Calculating lockMap route to: ".$maps_lut{$config{"lockMap_$ai_v{'lockMapIndex'}"}.'.rsw'}."(".$config{"lockMap_$ai_v{'lockMapIndex'}"}."): $ai_v{'temp'}{'randX'}, $ai_v{'temp'}{'randY'}") if ($config{'verbose'} && $sys{'Xmode'});
				} else {
					if ($ai_v{'temp'}{'randX'} ne "") {
						undef $ai_v{'temp'}{'randX'};
						undef $ai_v{'temp'}{'randY'};
					}
					print qq~Calculating lockMap route to: $maps_lut{$config{"lockMap_$ai_v{'lockMapIndex'}"}.'.rsw'}($config{"lockMap_$ai_v{'lockMapIndex'}"})\n~;
					injectMessage("Calculating lockMap route to: ".$maps_lut{$config{"lockMap_$ai_v{'lockMapIndex'}"}.'.rsw'}."(".$config{"lockMap_$ai_v{'lockMapIndex'}"}.")") if ($config{'verbose'} && $sys{'Xmode'});
				}
	...
	if ($config{'useWaypoint'} && $ai_seq[0] eq "" && @{$field{'field'}} > 1 && !$cities_lut{$field{'name'}.'.rsw'} && !$ai_v{'temp'}{'shopOpen'}) {
		...
		if ($config{'useWaypoint'}) {
			print "move to (".$route{"$route{'count'}"}{'x'}.",".$route{"$route{'count'}"}{'y'}.")\n";
...
#mod Stop
	...
	if ($config{'route_randomWalk'} && $ai_seq[0] eq "" && @{$field{'field'}} > 1 && !$cities_lut{$field{'name'}.'.rsw'}) {
		...
		print addTag($sys{'Vx_interface'},"route"),"Calculating random route to: $maps_lut{$field{'name'}.'.rsw'}($field{'name'}): $ai_v{'temp'}{'randX'}, $ai_v{'temp'}{'randY'}\n";
		$CONSOLE->Attr($ATTR_NORMAL) if ($config{'colorMode'} && !$sys{'Vx_interface'});
		injectMessage("Calculating random route to: $maps_lut{$field{'name'}.'.rsw'}($field{'name'}): $ai_v{'temp'}{'randX'}, $ai_v{'temp'}{'randY'}") if ($config{'verbose'} && $sys{'Xmode'});
	...
	if ($ai_seq[0] eq "dead" && $config{'deadRespawn'}
		...
		injectMessage("Disconnecting on death!") if ($config{'verbose'} && $sys{'Xmode'});
		quit();
	}
	...
	if (($ai_seq[0] eq "" || $ai_seq[0] eq "route" || $ai_seq[0] eq "route_getRoute" || $ai_seq[0] eq "route_getMapRoute" 
		...
		while (defined($config{"useSelf_item_$i"})) {
			...
			if ($chars[$config{'char'}]{'percent_hp'} <= $config{"useSelf_item_$i"."_hp_upper"} && $chars[$config{'char'}]{'percent_hp'} >= $config{"useSelf_item_$i"."_hp_lower"}
				&& $chars[$config{'char'}]{'percent_sp'} <= $config{"useSelf_item_$i"."_sp_upper"} && $chars[$config{'char'}]{'percent_sp'} >= $config{"useSelf_item_$i"."_sp_lower"}
				...
				&& (!$config{"useSelf_item_$i"."_inLockOnly"} || ($config{"useSelf_item_$i"."_inLockOnly"} && $field{'name'} eq $config{"lockMap_$ai_v{'lockMapIndex'}"}))
				&& (!$config{"useSelf_item_$i"."_inStatus"} || ($config{"useSelf_item_$i"."_inStatus"} && binFind(\@skillsST, $skillsST_lut{$config{"useSelf_item_$i"."_inStatus"}}) ne ""))
				&& (!$config{"useSelf_item_$i"."_outStatus"} || ($config{"useSelf_item_$i"."_outStatus"} && binFind(\@skillsST, $skillsST_lut{$config{"useSelf_item_$i"."_outStatus"}}) eq ""))
				) { 
...
#Auto useself skill
			if ($chars[$config{'char'}]{'percent_hp'} <= $config{"useSelf_skill_$i"."_hp_upper"} && $chars[$config{'char'}]{'percent_hp'} >= $config{"useSelf_skill_$i"."_hp_lower"}
				&& $chars[$config{'char'}]{'percent_sp'} <= $config{"useSelf_skill_$i"."_sp_upper"} && $chars[$config{'char'}]{'percent_sp'} >= $config{"useSelf_skill_$i"."_sp_lower"}
				&& $chars[$config{'char'}]{'sp'} >= $skillsSP_lut{$skills_rlut{lc($config{"useSelf_skill_$i"})}}{$config{"useSelf_skill_$i"."_lvl"}}
				&& (!$config{"useSelf_skill_$i"."_timeout"} || timeOut($ai_v{"useSelf_skill_$i"."_time"},$config{"useSelf_skill_$i"."_timeout"}))
				...
				&& (!$config{"useSelf_skill_$i"."_inLockOnly"} || ($config{"useSelf_skill_$i"."_inLockOnly"} && $field{'name'} eq $config{"lockMap_$ai_v{'lockMapIndex'}"}))
				&& (!$config{"useSelf_skill_$i"."_monsters"} || existsInList($config{"useSelf_skill_$i"."_monsters"}, $monsters{$ai_seq_args[0]{'ID'}}{'name'}))
				&& (!$config{"useSelf_skill_$i"."_inStatus"} || ($config{"useSelf_skill_$i"."_inStatus"} && binFind(\@skillsST, $skillsST_lut{$config{"useSelf_skill_$i"."_inStatus"}}) ne ""))
				&& (!$config{"useSelf_skill_$i"."_outStatus"} || ($config{"useSelf_skill_$i"."_outStatus"} && binFind(\@skillsST, $skillsST_lut{$config{"useSelf_skill_$i"."_outStatus"}}) eq ""))
				&& ($config{"useSelf_skill_$i"."_whileSitting"} || !$chars[$config{'char'}]{'sitting'})
				&& (defined($config{"useSelf_skill_$i"."_spirits_lower"}) && $chars[$config{'char'}]{'spirits'} >= $config{"useSelf_skill_$i"."_spirits_lower"})
				&& (defined($config{"useSelf_skill_$i"."_spirits_upper"}) && $chars[$config{'char'}]{'spirits'} <= $config{"useSelf_skill_$i"."_spirits_upper"})
				) {
	...
	if ( $config{'partySkill_0'} && %{$chars[$config{'char'}]{'party'}} && ($ai_seq[0] eq "" || $ai_seq[0] eq "route" || $ai_seq[0] eq "route_getRoute" || $ai_seq[0] eq "route_getMapRoute"
		...
		if ($ai_v{'partySkill_lvl'} > 0 && $partyTargetID) {
			print qq~Party Skill used ($chars[$config{'char'}]{'party'}{'users'}{$partyTargetID}{'name'})Skills Used: $skills_lut{$skills_rlut{lc($ai_v{'follow_skill'})}} (lvl $ai_v{'follow_skill_lvl'})\n~ if $config{'debug'};
...
# by SnT2k
	...
	$config{'minDistance'} >= distance(\%{$chars[$config{'char'}]{'pos_to'}}, \%{$monsters{$ai_v{'ai_attack_ID'}}{'pos_to'}})) {
		...
		injectMessage("Fleeing from target") if ($config{'verbose'} && $sys{'Xmode'});
	}
...
##### FOLLOW ##### 
	...
	if ($ai_seq[0] eq "follow" && $ai_seq_args[0]{'following'} && ($players{$ai_seq_args[0]{'ID'}}{'dead'} || $players_old{$ai_seq_args[0]{'ID'}}{'dead'})) {
		...
		injectMessage("Master died.  I'll wait here.") if ($config{'verbose'} && $sys{'Xmode'});
		...
		injectMessage("I lost my master") if ($config{'verbose'} && $sys{'Xmode'});
		undef $ai_seq_args[0]{'following'};
		if ($players_old{$ai_seq_args[0]{'ID'}}{'disconnected'}) {
			print "My master disconnected\n";
			injectMessage("My master disconnected") if ($config{'verbose'} && $sys{'Xmode'});
		} elsif ($players_old{$ai_seq_args[0]{'ID'}}{'disappeared'}) {
			print "Trying to find lost master\n";
			injectMessage("Trying to find lost master") if ($config{'verbose'} && $sys{'Xmode'});
			...
			injectMessage("Don't know what happened to Master") if ($config{'verbose'} && $sys{'Xmode'});
			undef $ai_seq_args[0]{'following'}; 
	...
	if ($ai_seq[0] eq "follow" && !$ai_seq_args[0]{'following'} && !$ai_seq_args[0]{'follow_lost_portalID'} &&  !$players{$ai_seq_args[0]{'ID'}}{'dead'}) {
		if ($chars[$config{'char'}]{'party'}{'users'}{$ai_seq_args[0]{'ID'}} ne "" && $chars[$config{'char'}]{'party'}{'users'}{$ai_seq_args[0]{'ID'}}{'online'} && $chars[$config{'char'}]{'party'}{'users'}{$ai_seq_args[0]{'ID'}}{'map'} ne "") {
			...
			if ($chars[$config{'char'}]{'party'}{'users'}{$ai_seq_args[0]{'ID'}}{'pos'}{'x'} > 0) {
				...
				print "CalculatingRoute: $maps_lut{$ai_seq_args[0]{'follow_lost_char'}{'map'}.'.rsw'}($ai_seq_args[0]{'follow_lost_char'}{'map'}): $ai_seq_args[0]{'follow_lost_char'}{'pos'}{'x'}, $ai_seq_args[0]{'follow_lost_char'}{'pos'}{'y'}\n";
				...
				print "Calculating Route: $maps_lut{$ai_seq_args[0]{'follow_lost_char'}{'map'}.'.rsw'}($ai_seq_args[0]{'follow_lost_char'}{'map'})\n";
				ai_route(\%{$ai_v{'temp'}{'returnHash'}}, $ai_seq_args[0]{'follow_lost_char'}{'pos'}{'x'}, $ai_seq_args[0]{'follow_lost_char'}{'pos'}{'y'}, $ai_seq_args[0]{'follow_lost_char'}{'map'}, 0, 0, 1, 0, 0, 1);
			}
		}
	}
...
##### FOLLOW-LOST ##### 
	...
	if ($ai_seq[0] eq "follow" && $ai_seq_args[0]{'ai_follow_lost'}) { 
		...
		if (timeOut(\%{$ai_seq_args[0]{'ai_follow_lost_end'}})) {
			...
			injectMessage("Couldn't find master, giving up") if ($config{'verbose'} && $sys{'Xmode'});
			...
			injectMessage("Found my master!") if ($config{'verbose'} && $sys{'Xmode'});
...
# Make Chat after sit
	if ($chars[$config{'char'}]{'sitting'} && !defined($timeout{'ai_makechatAuto'}{'time'}) && $config{'makeChatwhenSit'}) {
		...
		if ($title ne "") {
			sendChatRoomCreate(\$remote_socket, $title,20,0,vocalString(4));
...
#Reduce code size &  fix monster Priority
	...
	if ($ai_seq[0] eq "attack" && timeOut(\%{$ai_seq_args[0]{'ai_attack_giveup'}})) {
		...
		injectMessage("Can't reach or damage target, dropping target") if ($config{'verbose'} && $sys{'Xmode'});
		...
		if ($monsters_old{$ai_v{'ai_attack_ID_old'}}{'dead'}) {
			print addTag($sys{'Vx_interface'},"targetDie"),"Target died\n";
# exp report 
	...
	#Add Data to Monster Report
			...
			print "Target lost\n";
			injectMessage("Target lost") if ($config{'verbose'} && $sys{'Xmode'});
		}
		...
		if (!%{$ai_seq_args[0]{'attackMethod'}}) {
			...
			while ($config{"attackSkillSlot_$i"} ne "") {
				...
				if ($chars[$config{'char'}]{'percent_hp'} >= $config{"attackSkillSlot_$i"."_hp_lower"} && $chars[$config{'char'}]{'percent_hp'} <= $config{"attackSkillSlot_$i"."_hp_upper"}
					&& $chars[$config{'char'}]{'percent_sp'} >= $config{"attackSkillSlot_$i"."_sp_lower"} && $chars[$config{'char'}]{'percent_sp'} <= $config{"attackSkillSlot_$i"."_sp_upper"}
		...
		if ($chars[$config{'char'}]{'sitting'}) {
			...
			if (int($ai_seq_args[0]{'attackMethod'}{'distance'} / $ai_seq_args[0]{'distanceDivide'}) == 0
				...
				injectMessage("Dropping target - couldn't reach target") if ($config{'verbose'} && $sys{'Xmode'});
			} else {
			...
			if ($ai_seq_args[0]{'dest_map'} eq $field{'name'}
				...
				if (!(@{$ai_seq_args[0]{'mapSolution'}})) {
					...
					if (!%{$ai_seq_args[0]{'dest_field'}}) {
						getField("$def_field/$ai_seq_args[0]{'dest_map'}.fld", \%{$ai_seq_args[0]{'dest_field'}});
					}
...
#mod Stop
	...
	if ($ai_seq[0] eq "items_gather" && !%{$items{$ai_seq_args[0]{'ID'}}}) {
		...
		injectMessage("Failed to gather $items_old{$ai_seq_args[0]{'ID'}}{'name'} ($items_old{$ai_seq_args[0]{'ID'}}{'binID'}) : Lost target") if ($config{'verbose'} && $sys{'Xmode'});
		shift @ai_seq;
		shift @ai_seq_args;
		...
		if (timeOut(\%{$ai_seq_args[0]{'ai_items_gather_giveup'}})) {
			...
			injectMessage("Failed to gather $items{$ai_seq_args[0]{'ID'}}{'name'} ($items{$ai_seq_args[0]{'ID'}}{'binID'}) : Timeout") if ($config{'verbose'} && $sys{'Xmode'});
			...
			if ($sys{'Xmode'}) {
				...
				injectMessage("Failed to gather $items{$ai_seq_args[0]{'ID'}}{'name'} ($items{$ai_seq_args[0]{'ID'}}{'binID'}) : No looting!") if ($config{'verbose'} && $sys{'Xmode'});
			}
			shift @ai_seq;
			shift @ai_seq_args;
		}
	...
	if ($ai_seq[0] eq "take" && !%{$items{$ai_seq_args[0]{'ID'}}}) {
		...
		injectMessage("Failed to take $items{$ai_seq_args[0]{'ID'}}{'name'} ($items{$ai_seq_args[0]{'ID'}}{'binID'})") if ($config{'verbose'} && $sys{'Xmode'});
		$items{$ai_seq_args[0]{'ID'}}{'take_failed'}++;
		shift @ai_seq;
		shift @ai_seq_args;
	...
	if (timeOut(\%{$timeout{'ai_teleport_away'}}) && $ai_v{'ai_teleport_safe'}) {
		foreach (@monstersID) {
			...
			if ($mon_control{lc($monsters{$_}{'name'})}{'teleport_auto'}==1) {
				print "[Act] Avoiding monster : $monsters{$_}{'name'}\n";
				useTeleport(1);
				$ai_v{'temp'}{'search'} = 1;
				last;
			}
	...
	if ((($config{'teleportAuto_hp'} && $chars[$config{'char'}]{'percent_hp'} <= $config{'teleportAuto_hp'} && ai_getAggressives())
		...
		if ($config{'teleportAuto_hp'} && $chars[$config{'char'}]{'percent_hp'} <= $config{'teleportAuto_hp'}) {
			print "[Act] Teleport : Hp is lower than $config{'teleportAuto_hp'} ($chars[$config{'char'}]{'percent_hp'})\n";
		}else{
			print "[Act] Teleport : Aggressive is more than $config{'teleportAuto_minAggressives'} (".scalar(ai_getAggressives()).")\n";
		}
		useTeleport(1);
...
#teleport Search - Kladi improve
	...
	if ($ai_seq[0] eq "chatauto" && timeOut(\%{$ai_seq_args[0]})){
		if ($ai_seq_args[0]{'ans'} =~ /^e \d+/) {
			...
			sendMessage(\$remote_socket, "pm", $ai_seq_args[0]{'ans'},$ai_seq_args[0]{'name'});
		}else {
			sendMessage(\$remote_socket, "c", $ai_seq_args[0]{'ans'});
		}
		shift @ai_seq;
		shift @ai_seq_args;
	}
	...
	if ($ai_seq[0] eq "avoid") {
		if (!defined($ai_seq_args[0]{'type'})) {
			...
			print "[Act] Teleport/Respawn Success .. Quit ..\n";
			sysLog("D","** Teleport/Respawn Success .. Quit ..\n");
...
#avoiding players
		foreach (@playersID) {
			...
			if ($ppl_control{$players{$_}{'name'}}{'teleport_auto'} || $gid_control{$_}{'teleport_auto'}
				...
				print "[Act] Avoiding $players{$_}{'name'} ($players{$_}{'nameID'}) Distance: $dis , use teleport lv $args{'type'}\n";
				sysLog("D","**Avoiding $players{$_}{'name'} ($players{$_}{'nameID'}) Distance: $dis , use teleport lv $args{'type'}\n");
			...
			if ($ppl_control{$players{$_}{'name'}}{'disconnect_auto'} || $gid_control{$_}{'disconnect_auto'}
				...
				print "[Act] Avoiding $players{$_}{'name'} ($players{$_}{'nameID'}) Distance: $dis , Disconnect...\n";
				sysLog("D","**Avoiding $players{$_}{'name'} ($players{$_}{'nameID'}) Distance: $dis , Disconnect...\n");
				$timeout_ex{'master'}{'time'} = time;
				$timeout_ex{'master'}{'timeout'} = $config{'avoid_reConnect'};
				killConnection(\$remote_socket);
				last;
			...
			if ($config{'avoidGM'}==4 && (existsInPatternList($config{'avoid_namePattern'},$players{$_}{'name'})|| $GameMasters{$_}) &&  binFind(\@ai_seq, "avoid") eq "") {
				my $dis = int(distance(\%{$chars[$config{'char'}]{'pos_to'}}, \%{$players{$_}{'pos_to'}}));
				print "[Act] Avoiding $players{$_}{'name'} ($players{$_}{'nameID'}) Distance: $dist , Stand Still ...\n";
				sysLog("D","**Avoiding $players{$_}{'name'} ($players{$_}{'nameID'}) Distance: $dist , Stand Still ...\n");
				unshift @ai_seq, "avoid";
				unshift @ai_seq_args, {};
			}
			#Exclusive Avoid
			if ($config{'Exclusive_Avoid'}==1 && !$cities_lut{$field{'name'}.'.rsw'} && ($field{'name'} eq $config{"lockMap_$ai_v{'lockMapIndex'}"} || $config{"lockMap_$ai_v{'lockMapIndex'}"} eq "")
				...
				print "[Act] Force Exclusive Avoid : $players{$_}{'name'} ($players{$_}{'nameID'}) [$jobs_lut[$players{$_}{'jobID'}]]\n";
				sysLog("D", "**Force Exclusive Avoid : $players{$_}{'name'} ($players{$_}{'nameID'}) [$jobs_lut[$players{$_}{'jobID'}]]"."\n");
				useTeleport(1);
				$ai_v{'clear_aiQueue'} = 1;
				last;
...
# Display Ai Queue
	my $tbase =  sprintf("%.2f",$chars[$config{'char'}]{'exp'}/$chars[$config{'char'}]{'exp_max'} * 100) if $chars[$config{'char'}]{'exp_max'}; 
	my $tjob = sprintf("%.2f",$chars[$config{'char'}]{'exp_job'}/$chars[$config{'char'}]{'exp_job_max'} * 100) if $chars[$config{'char'}]{'exp_job_max'}; 
	my $tweight = int($chars[$config{'char'}]{'weight'}/$chars[$config{'char'}]{'weight_max'} * 100)."%" if $chars[$config{'char'}]{'weight_max'};
	if (!$sys{'Vx_interface'} && $^O eq 'MSWin32') {
		if ($sys{'title'} ne "$chars[$config{'char'}]{'name'} (B$chars[$config{'char'}]{'lv'}:$tbase J$chars[$config{'char'}]{'lv_job'}:$tjob) w$tweight $field{'name'} : $chars[$config{'char'}]{'pos_to'}{'x'},$chars[$config{'char'}]{'pos_to'}{'y'}") {
			$sys{'title'} = "$chars[$config{'char'}]{'name'} (B$chars[$config{'char'}]{'lv'}:$tbase J$chars[$config{'char'}]{'lv_job'}:$tjob) w$tweight $field{'name'} : $chars[$config{'char'}]{'pos_to'}{'x'},$chars[$config{'char'}]{'pos_to'}{'y'}";
			$CONSOLE->Title($sys{'title'});
		}
	}elsif ($sys{'Vx_interface'}) {
		if ($sys{'title'} ne "$chars[$config{'char'}]{'name'} (B$chars[$config{'char'}]{'lv'}:$tbase J$chars[$config{'char'}]{'lv_job'}:$tjob) w$tweight") {
			$sys{'title'} = "$chars[$config{'char'}]{'name'} (B$chars[$config{'char'}]{'lv'}:$tbase J$chars[$config{'char'}]{'lv_job'}:$tjob) w$tweight";
			Vx::setTitle($sys{'title'});
		}
		Vx::setAiText("@ai_seq");
	}
