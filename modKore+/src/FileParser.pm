...
sub load {
	my $r_array = (shift or \@parseFiles);
	foreach (@{$r_array}) {
		if (-e $$_{'file'}) {
			System::message("Loading $$_{'file'}...\n","info");
			&{$$_{'function'}}("$$_{'file'}", $$_{'hash'});
		} else {
			System::error("Error: Couldn't load $$_{'file'}\n","warn");
			undef $$_{'hash'};
		}
	}
}
...
sub parseMonControl {
	...
	foreach (<FILE>) {
		...
		if ($key ne "") {
			...
			$$r_hash{lc($key)}{'monitor_auto'} = $args[5];
		}
	}
	close FILE;
}
...
#	my $file = shift;
#	my $r_hash = shift;
#	$$r_hash = XMLin($file,suppressempty =>1);
#}
